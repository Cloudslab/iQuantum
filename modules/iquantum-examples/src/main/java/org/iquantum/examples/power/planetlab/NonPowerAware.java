package org.iquantum.examples.power.planetlab;
...
import org.iquantum.examples.power.Constants;
import org.iquantum.examples.power.Helper;
import org.iquantum.power.PowerCDatacenterNonPowerAware;
...
public class NonPowerAware {
	...
	public static void main(String[] args) throws IOException {
		...
		try {
			...
			CBroker broker = Helper.createBroker();
			int brokerId = broker.getId();

			List<CTask> CTaskList = PlanetLabHelper.createCloudletListPlanetLab(brokerId, inputFolder);
			List<Vm> vmList = Helper.createVmList(brokerId, CTaskList.size());
			List<PowerHost> hostList = Helper.createHostList(PlanetLabConstants.NUMBER_OF_HOSTS);

			PowerCDatacenterNonPowerAware datacenter = (PowerCDatacenterNonPowerAware) Helper.createDatacenter(
					"Datacenter",
					PowerCDatacenterNonPowerAware.class,
					hostList,
