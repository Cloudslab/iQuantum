package org.iquantum.examples.power;
...
import org.iquantum.power.PowerCBroker;
import org.iquantum.power.PowerHost;
import org.iquantum.power.PowerHostUtilizationHistory;
...
public class Helper {
	 ...
	 */
	public static CBroker createBroker() {
		CBroker broker = null;
		try {
			broker = new PowerCBroker("Broker");
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(0);
		}
	 ...
	 */
	public static CDatacenter createDatacenter(
			String name,
			Class<? extends CDatacenter> datacenterClass,
		...
		try {
			CDatacenter = datacenterClass.getConstructor(
					String.class,
					CDatacenterCharacteristics.class,
	 ...
	 */
	public static void printCloudletList(List<CTask> list) {
		int size = list.size();
		CTask CTask;
		...
		for (int i = 0; i < size; i++) {
			CTask = list.get(i);
			Log.print(indent + CTask.getCloudletId());

			if (CTask.getCloudletStatus() == CTask.SUCCESS) {
				Log.printLine(indent + "SUCCESS" + indent + indent + CTask.getResourceId() + indent
						+ CTask.getVmId() + indent + dft.format(CTask.getActualCPUTime()) + indent
						+ dft.format(CTask.getExecStartTime()) + indent + indent
