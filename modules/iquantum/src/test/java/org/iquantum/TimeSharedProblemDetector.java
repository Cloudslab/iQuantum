...
public class TimeSharedProblemDetector {
	...
	/** The cloudlet list. */
	private static List<CTask> cloudletList;
	...
	public static void main(String[] args) {
		...
		try {
			...
			CDatacenter datacenter0 = createDatacenter("Datacenter_0");
			...
			CTask cloudlet = new CTask(id, length, pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel);
			cloudlet.setUserId(brokerId);
			cloudlet.setVmId(vmid);
			
			CTask cloudlet1 = new CTask(1, length, pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel);
			cloudlet1.setUserId(brokerId);
			cloudlet1.setVmId(1);
			
			CTask cloudlet2 = new CTask(2, length, pesNumber, fileSize, outputSize, utilizationModel, utilizationModel, utilizationModel);
	...
	private static CDatacenter createDatacenter(String name) {
		...
		CDatacenter datacenter = null;
		try {
			datacenter = new CDatacenter(name, characteristics, new VmAllocationPolicySimple(hostList), storageList, 0);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private static void printCloudletList(List<CTask> list) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (cloudlet.getCloudletStatus() == CTask.SUCCESS) {
				Log.print("SUCCESS");
