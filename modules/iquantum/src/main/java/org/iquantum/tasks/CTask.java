...
public class CTask {
     ...
     * execution history on each VM is registered at {@link CTask#resList}
     */
    ...
    public void setUserId(final int id) {
        userId = id;
        if (record) {
            write("Assigns the Cloudlet to " + iQuantum.getEntityName(id) + " (ID #" + id + ")");
        }
    }
    ...
    public void setCloudletStatus(final int newStatus) throws Exception {
        ...
        if (newStatus < CTask.CREATED || newStatus > CTask.FAILED_RESOURCE_UNAVAILABLE) {
            throw new Exception(
                    "Cloudlet.setCloudletStatus() : Error - Invalid integer range for Cloudlet status.");
        }

        if (newStatus == CTask.SUCCESS) {
            finishTime = iQuantum.clock();
        }

        if (record) {
            write("Sets Cloudlet status from " + getCloudletStatusString() + " to "
                    + CTask.getStatusString(newStatus));
        }
    ...
    public String getCloudletStatusString() {
        return CTask.getStatusString(status);
    }
    ...
    public static String getStatusString(final int status) {
        ...
        switch (status) {
            ...
            case CTask.FAILED_RESOURCE_UNAVAILABLE:
