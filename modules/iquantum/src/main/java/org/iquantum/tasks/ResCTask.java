...
/**
 * iQuantum ResCloudlet represents a Cloudlet submitted to CloudResource for processing. This class
 ...
 */
public class ResCTask {

	/** The Cloudlet object. */
	private final CTask CTask;
	 ...
	 * is determined by {@link gridsim.iQuantum#clock()}.
	 * 
	 * @param CTask a cloudlet object
	 * @see gridsim.iQuantum#clock()
	 * @pre cloudlet != null
	 * @post $none
	 */
	public ResCTask(CTask CTask) {
	 ...
	 * @post $none
	 */
	public ResCTask(CTask CTask, long startTime, int duration, int reservID) {
		this.CTask = CTask;
	...
	private void init() {
		...
		pesNumber = CTask.getNumberOfPes();
		...
		CTask.setSubmissionTime(arrivalTime);
		...
		// hostList.
		cloudletFinishedSoFar = CTask.getCloudletFinishedSoFar() * Consts.MILLION;
	}
	...
	public long getCloudletLength() {
		return CTask.getCloudletLength();
	}
	...
	public long getCloudletTotalLength() {
		return CTask.getCloudletTotalLength();
	}
	...
	public boolean setCloudletStatus(int status) {
		...
		int prevStatus = CTask.getCloudletStatus();
		...
		try {
			...
			CTask.setCloudletStatus(status);

			// if a previous Cloudlet status is INEXEC
			if (prevStatus == CTask.INEXEC) {
				// and current status is either CANCELED, PAUSED or SUCCESS
				if (status == CTask.CANCELED || status == CTask.PAUSED || status == CTask.SUCCESS) {
			...
			if (prevStatus == CTask.RESUMED && status == CTask.SUCCESS) {
				// then update the Cloudlet completion time
				totalCompletionTime += (clock - startExecTime);
				return true;
			}
			...
			if (status == CTask.INEXEC || (prevStatus == CTask.PAUSED && status == CTask.RESUMED)) {
				startExecTime = clock;
				CTask.setExecStartTime(startExecTime);
			}
	...
	public void setExecParam(double wallClockTime, double actualCPUTime) {
		CTask.setExecParam(wallClockTime, actualCPUTime);
	}
	 ...
	 * @post $none
         ...
         * At {@link  CloudletSchedulerTimeShared#cloudletSubmit(CTask)}
	...
	public long getRemainingCloudletLength() {
		long length = CTask.getCloudletTotalLength() * Consts.MILLION - cloudletFinishedSoFar;
	...
	public int getCloudletStatus() {
		return CTask.getCloudletStatus();
	}
