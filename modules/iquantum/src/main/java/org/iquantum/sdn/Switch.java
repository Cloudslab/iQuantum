...
public class Switch extends SimEntity implements Node{
	...
	private void internalPackageProcessing() {
		if(updatePackageProcessing()) {
			...
			System.err.println(iQuantum.clock() + ": " + getName() +": Nothing changed! omg");
			sendInternalEvent();
		}
	}
	...
	private boolean updatePackageProcessing() {
		...
		double timeSpent = iQuantum.round(currentTime - this.previousTime);
		...
		System.err.println(iQuantum.clock()+": Switch.updatePackageProcessing("+getName()+ ") #("+this.processingTable.size()+"):Time spent:"+timeSpent+
	...
	private void processPackageFinish(Package pkg) {
		Log.printLine(iQuantum.clock() + ": " + getName() + ": finished processing a package:" + pkg);
		nos.sendPackageToNextHop(this, pkg);
	}

	private void sendToBuffer(Package pkg) {
		Log.printLine(iQuantum.clock() + ": " + getName() + ": Package received from Network:" + pkg);
	...
	private double nextFinishTime() {
		...
		delay=iQuantum.round(delay);
		
		if (delay < iQuantum.getMinTimeBetweenEvents()) { 
			Log.printLine(iQuantum.clock() + ": " + getName() + ": delay is too short: "+ delay);
			delay = iQuantum.getMinTimeBetweenEvents();
		}
