...
import org.iquantum.sdn.example.policies.VmSchedulerTimeSharedEnergy;
...
public abstract class NetworkOperatingSystem extends SimEntity {
	...
	public void addPackageToChannel(Node sender, Package pkg) {
		...
		if(sender.equals(sender.getVMRoute(src, dst, flowId))) {
			...
			//Log.printLine(iQuantum.clock() + ": " + getName() + ".addPackageToChannel: Loopback package: "+pkg +". Send to destination:"+dst);
		...
		Log.printLine(iQuantum.clock() + ": " + getName() + ".addPackageToChannel ("+channel
				+"): Transmission added:" + 
	...
	private void sendInternalEvent() {
		iQuantum.cancelAll(getId(), new PredicateType(Constants.SDN_INTERNAL_PACKAGE_PROCESS));
		
		if(channelTable.size() != 0) {
			// More to process. Send event again
			double delay = this.nextFinishTime();
			Log.printLine(iQuantum.clock() + ": " + getName() + ".sendInternalEvent(): next finish time: "+ delay);
	...
	private void processCompletePackages(List<Channel> channels){
		for(Channel ch:channels) {
			...
			for (Transmission tr:ch.getArrivedPackages()){
				...
				Log.printLine(iQuantum.clock() + ": " + getName() + ": Package completed: "+pkg +". Send to destination:"+dest);
	...
	private Channel createChannel(int src, int dst, int flowId, Node srcNode) {
		...
		if(flowId != -1 && lowestBw < reqBw) {
			...
			// Cannot make channel.
			Log.printLine(iQuantum.clock() + ": " + getName() + ": Free bandwidth is less than required.("+getKey(src,dst,flowId)+"): ReqBW="+ reqBw + "/ Free="+lowestBw);
			//return null;
		}
	...
	public int getHostAddressByVmId(int vmId) {
		Vm vm = findVm(vmId);
		if(vm == null) {
			Log.printLine(iQuantum.clock() + ": " + getName() + ": Cannot find VM with vmId = "+ vmId);
			return -1;
		}
		...
		if(sdnhost == null) {
			Log.printLine(iQuantum.clock() + ": " + getName() + ": Cannot find SDN Host with vmId = "+ vmId);
			return -1;
		}
