...
/**
 * CloudimShutdown waits for termination of all iQuantum user entities to determine the end of
 * simulation. This class will be created by iQuantum upon initialisation of the simulation, i.e.
 * done via <tt>iQuantum.init()</tt> method. Hence, do not need to worry about creating an object of
 ...
 */
public class iQuantumShutdown extends SimEntity {

	/** The total number of cloud users. */
	private int numUser;

	/**
	 * Instantiates a new iQuantumShutdown object.
	 * <p/>
	 ...
	 * cloud user entities. Otherwise, iQuantum program will hang or encounter a weird behaviour.
	 * 
	 ...
	 * @throws Exception when creating this entity before initialising iQuantum package
	 *             or this entity name is <tt>null</tt> or empty
	 * @see iQuantum#init(int, java.util.Calendar, boolean)
	 * @pre name != null
	 * @pre numUser >= 0
	 * @post $none
         * 
	 ...
	 */
	public iQuantumShutdown(String name, int numUser) throws Exception {
		// NOTE: This entity doesn't use any I/O port.
		// super(name, iQuantumTags.DEFAULT_BAUD_RATE);
		super(name);
	 ...
	 * correct. Otherwise, iQuantum package hangs forever or it does not terminate properly.
	 * 
	 * @param ev the ev
	 * @pre $none
	 * @post $none
	...
	public void processEvent(SimEvent ev) {
		numUser--;
		if (numUser == 0 || ev.getTag() == iQuantumTags.ABRUPT_END_OF_SIMULATION) {
			iQuantum.abruptallyTerminate();
		}
	}
