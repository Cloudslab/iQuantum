 ...
 */
public class ResourceInformationService extends SimEntity {
	...
	/** A list containing the id of all entities that are registered at the 
         ...
         * that the list is updated when a iQuantumTags.REGISTER_RESOURCE event
         * is received. However, only the Datacenter class sends and event
         * of this type, including its id as parameter.
	     * ---------------------
	     * Hoa's comments (July 2023): This is a list of datacenter id's.
         */
	 ...
	 */
	public ResourceInformationService(String name) throws Exception {
		super(name);
	...
	public void processEvent(SimEvent ev) {
		...
		// storing regional CIS id
			case iQuantumTags.REGISTER_REGIONAL_GIS:
				gisList.add((Integer) ev.getData());
				break;

			// request for all regional CIS list
			case iQuantumTags.REQUEST_REGIONAL_GIS:
			...
			// A Broker is requesting for a list of all hostList.
			case iQuantumTags.RESOURCE_AR_LIST:
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		Log.printLine("CloudInformationSevice.processOtherEvent(): " + "Unable to handle a request from "
				+ iQuantum.getEntityName(ev.getSource()) + " with event tag = " + ev.getTag());
	}
	...
	private void notifyAllEntity() {
		Log.printConcatLine(super.getName(), ": Notify all iQuantum entities for shutting down.");
	...
	/**
	 * Sends a {@link iQuantumTags#END_OF_SIMULATION} signal to all entity IDs
	...
	protected void signalShutdown(Collection<Integer> list) {
		...
		while (it.hasNext()) {
			obj = it.next();
			id = obj.intValue();
			super.send(id, 0L, iQuantumTags.END_OF_SIMULATION);
		}
	}
