 ...
 */
public class CDatacenter extends SimEntity {
	 ...
	 *  <ul>
	 *    <li>creating this entity before initializing iQuantum package
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		...
		if (ack) {
			...
			send(vm.getUserId(), iQuantum.getMinTimeBetweenEvents(), iQuantumTags.VM_CREATE_ACK, data);
		}
		...
		if (result) {
			...
			vm.updateVmProcessing(iQuantum.clock(), getVmAllocationPolicy().getHost(vm).getVmScheduler()
	...
	protected void processVmDestroy(SimEvent ev, boolean ack) {
		...
		if (ack) {
			...
			sendNow(vm.getUserId(), iQuantumTags.VM_DESTROY_ACK, data);
		}
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		// get the cloudlet
		CTask cl = getVmAllocationPolicy().getHost(vmId, userId).getVm(vmId,userId)
	...
	protected void updateCloudletProcessing() {
		...
		if (iQuantum.clock() < 0.111 || iQuantum.clock() >= getLastProcessTime() + iQuantum.getMinTimeBetweenEvents()) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				double time = host.updateVmsProcessing(iQuantum.clock());
			...
			// Schedule next processing event with iQuantum simulation engine
	 ...
	 */
	protected CDatacenterCharacteristics getCharacteristics() {
	 ...
	 */
	protected void setCharacteristics(CDatacenterCharacteristics characteristics) {
