package org.iquantum.backends.classical.container.resourceAllocatorMigrationEnabled;
...
import org.iquantum.backends.classical.container.lists.PowerContainerList;
import org.iquantum.backends.classical.container.resourceAllocators.PowerContainerAllocationPolicy;
...
//                    vm.setInWaiting(true);
    ...
    protected List<Map<String, Object>> findMapInSwitchedOffHosts(List<Container> containerList) {
        Log.print(String.format(" %s :  Find Placement in the switched of hosts", iQuantum.clock()));
...
//        Log.printConcatLine("Checking if the vm is over utilized or not!");
        for (Container container : vm.getContainerList()) {
            util += container.getTotalUtilizationOfCpuMips(iQuantum.clock());
        }
