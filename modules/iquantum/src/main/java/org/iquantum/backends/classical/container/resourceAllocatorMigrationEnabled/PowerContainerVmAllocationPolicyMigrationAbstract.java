...
import org.iquantum.backends.classical.container.resourceAllocators.PowerContainerVmAllocationAbstract;
import org.iquantum.backends.classical.container.vmSelectionPolicies.PowerContainerVmSelectionPolicy;
import org.iquantum.backends.classical.container.lists.PowerContainerVmList;
...
public abstract class PowerContainerVmAllocationPolicyMigrationAbstract extends PowerContainerVmAllocationAbstract {
    ...
    protected void addHistoryEntry(ContainerHostDynamicWorkload host, double metric) {
        ...
        if (!getTimeHistory().get(hostId).contains(iQuantum.clock())) {
            getTimeHistory().get(hostId).add(iQuantum.clock());
