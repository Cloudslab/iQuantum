...
//        Log.printLine("Power data center is Updating the cloudlet processing");
        ...
        if (currentTime > getLastProcessTime()) {
            ...
            if (!isDisableVmMigrations()) {
                ...
                Log.printConcatLine(iQuantum.clock(), ": The number of Migrations is:  ", getVmMigrationCount() - previousMigrationCount);
//                String[] msg={Double.toString (iQuantum.clock()), Integer.toString (getVmMigrationCount() - previousMigrationCount)  } ;                   // <--declared statement
//                try {
//                    getVmMigrationWriter().writeTofile(msg);
//                } catch (IOException e) {
//                    e.printStackTrace();
//                }
//                Log.printConcatLine(iQuantum.clock(),": The total number of Migrations is:  ",getVmMigrationCount());
            }
...
//        Log.printLine("Power data center is Updating CloudetProcessingWithoutSchedulingFutureEvents");
        if (iQuantum.clock() > getLastProcessTime()) {
...
//        Log.printLine("Power data center is Updating CloudetProcessingWithoutSchedulingFutureEventsForce");
    ...
    /*
     * (non-Javadoc)
     * @see org.iquantum.datacenters.Datacenter#processVmMigrate(org.iquantum.core.SimEvent,
     * boolean)
     */
    @Override
    protected void processVmMigrate(SimEvent ev, boolean ack) {
        ...
        SimEvent event = iQuantum.findFirstDeferred(getId(), new PredicateType(iQuantumTags.VM_MIGRATE));
        if (event == null || event.eventTime() > iQuantum.clock()) {
...
//        SimEvent event = iQuantum.findFirstDeferred(getId(), new PredicateType(containeriQuantumCloudTags.CONTAINER_MIGRATE));
//        if (event == null || event.eventTime() > iQuantum.clock()) {
//            updateCloudetProcessingWithoutSchedulingFutureEventsForce();
//        }
//    }
    /*
     * (non-Javadoc)
     * @see iQuantum.Datacenter#processCloudletSubmit(iQuantum.core.SimEvent, boolean)
     */
    @Override
    protected void processCloudletSubmit(SimEvent ev, boolean ack) {
//        Log.printLine("Power data center is processing cloudlet submit");
        super.processCloudletSubmit(ev, ack);
//        Log.printLine("Power Data Center : Set Cloudlet Submited to " + iQuantum.clock());
        setCloudletSubmitted(iQuantum.clock());
    }
