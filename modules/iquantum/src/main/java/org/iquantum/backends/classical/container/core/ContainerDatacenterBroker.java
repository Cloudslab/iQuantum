...
import org.iquantum.backends.classical.container.lists.ContainerList;
import org.iquantum.backends.classical.container.lists.ContainerVmList;
...
/**
     ...
     * The cloudlet list.
     */
    protected List<? extends ContainerCTask> cloudletList;
    /**
    * The container list
     */
     ...
     */
    protected List<? extends ContainerCTask> cloudletSubmittedList;

    /**
     * The cloudlet received list.
     */
    protected List<? extends ContainerCTask> cloudletReceivedList;
 ...
 /**
    ...
    public ContainerDatacenterBroker(String name, double overBookingfactor) throws Exception {
        ...
        setCloudletList(new ArrayList<ContainerCTask>());
        setCloudletSubmittedList(new ArrayList<ContainerCTask>());
        setCloudletReceivedList(new ArrayList<ContainerCTask>());
     ...
     * @post $none
     */
    public void submitCloudletList(List<? extends ContainerCTask> list) {
...
//        Log.printConcatLine("The Vm ID is ",  CloudletList.getById(getCloudletList(), cloudletId).getVmId(), "should be", vmId);
    ...
    public void processEvent(SimEvent ev) {
        switch (ev.getTag()) {
            ...
            case containeriQuantumTags.VM_NEW_CREATE:
            ...
            case iQuantumTags.END_OF_SIMULATION:
                shutdownEntity();
                break;
            case containeriQuantumTags.CONTAINER_CREATE_ACK:
...
//            ContainerVm p= ContainerVmList.getById(getVmsCreatedList(), vmId);
            ...
            Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": The Container #", containerId,
            ...
            Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Failed Creation of Container #", containerId);
        }
...
//                getDatacenterIdsList().size(), " resource(s)");
    ...
    protected void processNewVmCreate(SimEvent ev) {
        ...
        if (result == iQuantumTags.TRUE) {
            ...
            Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": VM #", vmId,
                    " has been created in Datacenter #", datacenterId, ", Host #",
                    ContainerVmList.getById(getVmsCreatedList(), vmId).getHost().getId());
        } else {
            Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Creation of VM #", vmId,
...
//                    Log.printLine(iQuantum.clock() + ": " + getName()
...
//        }
    ...
    protected void processCloudletReturn(SimEvent ev) {
        ContainerCTask cloudlet = (ContainerCTask) ev.getData();
        getCloudletReceivedList().add(cloudlet);
        Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Cloudlet ", cloudlet.getCloudletId(),
                " returned");
        Log.printConcatLine(iQuantum.clock(), ": ", getName(), "The number of finished Cloudlets is:", getCloudletReceivedList().size());
        cloudletsSubmitted--;
    ...
    protected void createVmsInDatacenter(int datacenterId) {
        ...
        int requestedVms = 0;
        String datacenterName = iQuantum.getEntityName(datacenterId);
        for (ContainerVm vm : getVmList()) {
            if (!getVmsToDatacentersMap().containsKey(vm.getId())) {
                Log.printLine(String.format("%s: %s: Trying to Create VM #%d in %s", iQuantum.clock(), getName(), vm.getId(), datacenterName));
                sendNow(datacenterId, iQuantumTags.VM_CREATE_ACK, vm);
                requestedVms++;
            }
        }
    ...
    protected void submitCloudlets() {
        ...
        List<ContainerCTask> successfullySubmitted = new ArrayList<>();
        for (ContainerCTask cloudlet : getCloudletList()) {
...
//                    }
                    containerIndex++;
                    sendNow(getDatacenterIdsList().get(0), iQuantumTags.CLOUDLET_SUBMIT, cloudlet);
                    cloudletsSubmitted++;
...
//                    if (!Log.isDisabled()) {
//                        Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Postponing execution of cloudlet ",
...
//
//            if (!Log.isDisabled()) {
//                Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Sending cloudlet ",
...
//            sendNow(getVmsToDatacentersMap().get(vm.getId()), iQuantumTags.CLOUDLET_SUBMIT, cloudlet);
//            cloudletsSubmitted++;
...
//            Log.printConcatLine(iQuantum.clock(), ": " + getName(), ": Destroying VM #", vm.getId());
    ...
    protected void submitContainers(){
        ...
        for(Container container:getContainerList()) {
            ContainerCTask cloudlet = getCloudletList().get(i);
                //Log.printLine("Containers Created" + getContainersCreated());
...
//                        Log.printConcatLine("Binding Cloudlet: ", cloudlet.getCloudletId(), "To Container: ",container.getId() , "Now it is", cloudlet.getContainerId());
        ...
        sendNow(getDatacenterIdsList().get(0), containeriQuantumTags.CONTAINER_SUBMIT, successfullySubmitted);

//        List<Container> successfullySubmitted = new ArrayList<>();
//        for (Container container : getContainerList()) {
//
//            if (!Log.isDisabled()) {
//                Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Sending container ",
...
//        }
     ...
     */
    @SuppressWarnings("unchecked")
    public <T extends ContainerCTask> List<T> getCloudletList() {
     ...
     */
    protected <T extends ContainerCTask> void setCloudletList(List<T> cloudletList) {
     ...
     */
    @SuppressWarnings("unchecked")
    public <T extends ContainerCTask> List<T> getCloudletSubmittedList() {
     ...
     */
    protected <T extends ContainerCTask> void setCloudletSubmittedList(List<T> cloudletSubmittedList) {
     ...
     */
    @SuppressWarnings("unchecked")
    public <T extends ContainerCTask> List<T> getCloudletReceivedList() {
     ...
     */
    protected <T extends ContainerCTask> void setCloudletReceivedList(List<T> cloudletReceivedList) {
