...
import org.iquantum.utils.datacloud.DataCloudTags;
import org.iquantum.network.InfoPacket;
import org.iquantum.tasks.CTask;
import org.iquantum.utils.datacloud.File;
...
public class ContainerDatacenter extends SimEntity {
    ...
    public void processEvent(SimEvent ev) {
        ...
        switch (ev.getTag()) {
            ...
            case iQuantumTags.RESOURCE_DYNAMICS:
                srcId = ((Integer) ev.getData()).intValue();
                sendNow(srcId, ev.getTag(), 0);
                break;

            case iQuantumTags.RESOURCE_NUM_PE:
                srcId = ((Integer) ev.getData()).intValue();
                int numPE = getCharacteristics().getNumberOfPes();
                sendNow(srcId, ev.getTag(), numPE);
                break;

            case iQuantumTags.RESOURCE_NUM_FREE_PE:
                srcId = ((Integer) ev.getData()).intValue();
            ...
            case iQuantumTags.CLOUDLET_SUBMIT:
                processCloudletSubmit(ev, false);
                break;

            // New Cloudlet arrives, but the sender asks for an ack
            case iQuantumTags.CLOUDLET_SUBMIT_ACK:
                processCloudletSubmit(ev, true);
                break;

            // Cancels a previously submitted Cloudlet
            case iQuantumTags.CLOUDLET_CANCEL:
                processCloudlet(ev, iQuantumTags.CLOUDLET_CANCEL);
                break;

            // Pauses a previously submitted Cloudlet
            case iQuantumTags.CLOUDLET_PAUSE:
                processCloudlet(ev, iQuantumTags.CLOUDLET_PAUSE);
                break;

            // Pauses a previously submitted Cloudlet, but the sender
            // asks for an acknowledgement
            case iQuantumTags.CLOUDLET_PAUSE_ACK:
                processCloudlet(ev, iQuantumTags.CLOUDLET_PAUSE_ACK);
                break;

            // Resumes a previously submitted Cloudlet
            case iQuantumTags.CLOUDLET_RESUME:
                processCloudlet(ev, iQuantumTags.CLOUDLET_RESUME);
                break;

            // Resumes a previously submitted Cloudlet, but the sender
            // asks for an acknowledgement
            case iQuantumTags.CLOUDLET_RESUME_ACK:
                processCloudlet(ev, iQuantumTags.CLOUDLET_RESUME_ACK);
                break;

            // Moves a previously submitted Cloudlet to a different resource
            case iQuantumTags.CLOUDLET_MOVE:
                processCloudletMove((int[]) ev.getData(), iQuantumTags.CLOUDLET_MOVE);
                break;

            // Moves a previously submitted Cloudlet to a different resource
            case iQuantumTags.CLOUDLET_MOVE_ACK:
                processCloudletMove((int[]) ev.getData(), iQuantumTags.CLOUDLET_MOVE_ACK);
                break;

            // Checks the status of a Cloudlet
            case iQuantumTags.CLOUDLET_STATUS:
                processCloudletStatus(ev);
                break;

            // Ping packet
            case iQuantumTags.INFOPKT_SUBMIT:
                processPingRequest(ev);
                break;

            case iQuantumTags.VM_CREATE:
                processVmCreate(ev, false);
                break;

            case iQuantumTags.VM_CREATE_ACK:
                processVmCreate(ev, true);
                break;

            case iQuantumTags.VM_DESTROY:
                processVmDestroy(ev, false);
                break;

            case iQuantumTags.VM_DESTROY_ACK:
                processVmDestroy(ev, true);
                break;

            case iQuantumTags.VM_MIGRATE:
                processVmMigrate(ev, false);
                break;

            case iQuantumTags.VM_MIGRATE_ACK:
                processVmMigrate(ev, true);
                break;

            case iQuantumTags.VM_DATA_ADD:
                processDataAdd(ev, false);
                break;

            case iQuantumTags.VM_DATA_ADD_ACK:
                processDataAdd(ev, true);
                break;

            case iQuantumTags.VM_DATA_DEL:
                processDataDelete(ev, false);
                break;

            case iQuantumTags.VM_DATA_DEL_ACK:
                processDataDelete(ev, true);
                break;

            case iQuantumTags.VM_DATACENTER_EVENT:
                updateCloudletProcessing();
                checkCloudletCompletion();
                break;
            case containeriQuantumTags.CONTAINER_SUBMIT:
                processContainerSubmit(ev, true);
                break;

            case containeriQuantumTags.CONTAINER_MIGRATE:
    ...
    public void processContainerSubmit(SimEvent ev, boolean ack) {
        ...
        for (Container container : containerList) {
            ...
            if (ack) {
                ...
                if (result) {
                    data[2] = iQuantumTags.TRUE;
                } else {
                    data[2] = iQuantumTags.FALSE;
                }
                if (result) {
...
//                    Log.printConcatLine("Assigning the container#" + container.getUid() + "to VM #" + containerVm.getUid());
                    ...
                    container.updateContainerProcessing(iQuantum.clock(), getContainerAllocationPolicy().getContainerVm(container).getContainerScheduler().getAllocatedMipsForContainer(container));
                } else {
                    data[0] = -1;
                ...
                send(ev.getSource(), iQuantum.getMinTimeBetweenEvents(), containeriQuantumTags.CONTAINER_CREATE_ACK, data);
    ...
    protected void processDataAdd(SimEvent ev, boolean ack) {
         ...
         * " from " + iQuantum.getEntityName(sentFrom));
         *******/
    ...
    protected void processPingRequest(SimEvent ev) {
        ...
        pkt.setTag(iQuantumTags.INFOPKT_RETURN);
        pkt.setDestId(pkt.getSrcId());

        // sends back to the sender
        sendNow(pkt.getSrcId(), iQuantumTags.INFOPKT_RETURN, pkt);
    }
    ...
    protected void processCloudletStatus(SimEvent ev) {
        ...
        // if a sender using normal send() methods
            ...
            try {
                ContainerCTask cl = (ContainerCTask) ev.getData();
                ...
                Log.printConcatLine(getName(), ": Error in processing iQuantumTags.CLOUDLET_STATUS");
        ...
        int tag = iQuantumTags.CLOUDLET_STATUS;
    ...
    protected void processVmCreate(SimEvent ev, boolean ack) {
        ...
        if (ack) {
            ...
            send(containerVm.getUserId(), iQuantum.getMinTimeBetweenEvents(), iQuantumTags.VM_CREATE_ACK, data);
        }
        ...
        if (result) {
            ...
            containerVm.updateVmProcessing(iQuantum.clock(), getVmAllocationPolicy().getHost(containerVm).getContainerVmScheduler()
    ...
    protected void processVmDestroy(SimEvent ev, boolean ack) {
        ...
        if (ack) {
            ...
            sendNow(containerVm.getUserId(), iQuantumTags.VM_DESTROY_ACK, data);
        }
    ...
    protected void processVmMigrate(SimEvent ev, boolean ack) {
        ...
        if (ack) {
            ...
            sendNow(ev.getSource(), iQuantumTags.VM_CREATE_ACK, data);
        }
    ...
    protected void processContainerMigrate(SimEvent ev, boolean ack) {
        ...
        if (ack) {
            ...
            sendNow(ev.getSource(), containeriQuantumTags.CONTAINER_CREATE_ACK, data);
        }
    ...
    protected void processCloudletMove(int[] receivedData, int type) {
        ...
        // get the cloudlet
        CTask cl = getVmAllocationPolicy().getHost(vmId, userId).getContainerVm(vmId, userId).getContainer(containerId, userId)
        ...
        if (cl == null) {// cloudlet doesn't exist
            ...
            if (cl.getCloudletStatusString().equals("Success")) {// if yes, send it back to user
                ...
                data[1] = cloudletId;
                data[2] = 0;
                sendNow(cl.getUserId(), iQuantumTags.CLOUDLET_SUBMIT_ACK, data);
            ...
            if (destId == getId()) {
                ...
                int tag = ((type == iQuantumTags.CLOUDLET_MOVE_ACK) ? iQuantumTags.CLOUDLET_SUBMIT_ACK
                        : iQuantumTags.CLOUDLET_SUBMIT);
                sendNow(destId, tag, cl);
            }
        }

        if (type == iQuantumTags.CLOUDLET_MOVE_ACK) {// send ACK if requested
            int[] data = new int[3];
            data[0] = getId();
            data[1] = cloudletId;
            if (failed) {
    ...
    protected void processCloudletSubmit(SimEvent ev, boolean ack) {
        ...
        try {
            ...
            if (cl.isFinished()) {
                String name = iQuantum.getEntityName(cl.getUserId());
                ...
                if (ack) {
                    ...
                    int tag = iQuantumTags.CLOUDLET_SUBMIT_ACK;
            ...
            if (estimatedFinishTime > 0.0 && !Double.isInfinite(estimatedFinishTime)) {
                ...
                send(getId(), estimatedFinishTime, iQuantumTags.VM_DATACENTER_EVENT);
            }
    ...
    protected void processCloudletResume(int cloudletId, int userId, int vmId, int containerId, boolean ack) {
        ...
        if (eventTime > 0.0) { // if this cloudlet is in the exec queue
            status = true;
            if (eventTime > iQuantum.clock()) {
                schedule(getId(), eventTime, iQuantumTags.VM_DATACENTER_EVENT);
            }
        }
        ...
        if (ack) {
            ...
            sendNow(userId, iQuantumTags.CLOUDLET_RESUME_ACK, data);
        }
    }
    ...
    protected void processCloudletPause(int cloudletId, int userId, int vmId, int containerId, boolean ack) {
        ...
        if (ack) {
            ...
            sendNow(userId, iQuantumTags.CLOUDLET_PAUSE_ACK, data);
        }
    }
    ...
    protected void processCloudletCancel(int cloudletId, int userId, int vmId, int containerId) {
        CTask cl = getVmAllocationPolicy().getHost(vmId, userId).getContainerVm(vmId, userId).getContainer(containerId, userId)
                .getContainerCloudletScheduler().cloudletCancel(cloudletId);
        sendNow(userId, iQuantumTags.CLOUDLET_CANCEL, cl);
    }
    ...
    protected void updateCloudletProcessing() {
        ...
        if (iQuantum.clock() < 0.111 || iQuantum.clock() > getLastProcessTime() + iQuantum.getMinTimeBetweenEvents()) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                double time = host.updateContainerVmsProcessing(iQuantum.clock());
            ...
            if (smallerTime < iQuantum.clock() + iQuantum.getMinTimeBetweenEvents() + 0.01) {
                smallerTime = iQuantum.clock() + iQuantum.getMinTimeBetweenEvents() + 0.01;
            }
            if (smallerTime != Double.MAX_VALUE) {
                schedule(getId(), (smallerTime - iQuantum.clock()), iQuantumTags.VM_DATACENTER_EVENT);
            }
            ...
            for (ContainerVm vm : host.getVmList()) {
                for (Container container : vm.getContainerList()) {
                    while (container.getContainerCloudletScheduler().isFinishedCloudlets()) {
                        CTask cl = container.getContainerCloudletScheduler().getNextFinishedCloudlet();
                        if (cl != null) {
                            sendNow(cl.getUserId(), iQuantumTags.CLOUDLET_RETURN, cl);
                        }
                    }
    ...
    /*
     * (non-Javadoc)
     * @see iQuantum.core.SimEntity#shutdownEntity()
     */
    @Override
    ...
    /*
     * (non-Javadoc)
     * @see iQuantum.core.SimEntity#startEntity()
     */
    @Override
    public void startEntity() {
        ...
        int gisID = iQuantum.getEntityId(regionalCisName);
        if (gisID == -1) {
            gisID = iQuantum.getCloudInfoServiceEntityId();
        }

        // send the registration to GIS
        sendNow(gisID, iQuantumTags.REGISTER_RESOURCE, getId());
