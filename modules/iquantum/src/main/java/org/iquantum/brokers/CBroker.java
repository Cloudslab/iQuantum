...
public class CBroker extends SimEntity {
	...
	/** The list of cloudlet submitted to the broker. 
         ...
         */
	protected List<? extends CTask> cloudletList;

	/** The list of submitted cloudlets. */
	protected List<? extends CTask> cloudletSubmittedList;

	/** The list of received cloudlet. */
	protected List<? extends CTask> cloudletReceivedList;
	...
	protected Map<Integer, CDatacenterCharacteristics> datacenterCharacteristicsList;
	 ...
	 * @post $none
	 */
	public CBroker(String name) throws Exception {
		super(name);

		setVmList(new ArrayList<Vm>());
		setVmsCreatedList(new ArrayList<Vm>());
		setCloudletList(new ArrayList<CTask>());
	 ...
	 * @post $none
         ...
         * be checked too.
	 */
	public void submitCloudletList(List<? extends CTask> list) {
	...
	protected void submitCloudlets() {
		int vmIndex = 0;
		List<CTask> successfullySubmitted = new ArrayList<CTask>();
		for (CTask CTask : getCloudletList()) {
			Vm vm;
			// if user didn't bind this cloudlet and it has not been executed yet
			if (CTask.getVmId() == -1) {
				vm = getVmsCreatedList().get(vmIndex);
			} else { // submit to the specific vm
				vm = VmList.getById(getVmsCreatedList(), CTask.getVmId());
				if (vm == null) { // vm was not created
					if(!Log.isDisabled()) {				    
					    Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Postponing execution of cloudlet ",
							CTask.getCloudletId(), ": bount VM not available");
					}
					continue;
				}
			}
			...
			if (!Log.isDisabled()) {
			    Log.printConcatLine(iQuantum.clock(), ": ", getName(), ": Sending cloudlet ",
					CTask.getCloudletId(), " to VM #", vm.getId());
			}
			
			CTask.setVmId(vm.getId());
			sendNow(getVmsToDatacentersMap().get(vm.getId()), iQuantumTags.CLOUDLET_SUBMIT, CTask);
			cloudletsSubmitted++;
			vmIndex = (vmIndex + 1) % getVmsCreatedList().size();
			getCloudletSubmittedList().add(CTask);
			successfullySubmitted.add(CTask);
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public <T extends CTask> List<T> getCloudletList() {
	 ...
	 */
	protected <T extends CTask> void setCloudletList(List<T> cloudletList) {
