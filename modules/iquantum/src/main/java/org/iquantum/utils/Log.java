...
public class Log {
    ...
    /**
     * Prints the concatenated text representation of the arguments and a new line, round up the Clock value to 2 decimal places.
     *
     * @param messages the messages to print
     */
    public static void printConcatLine(Object... messages) {
        if (!isDisabled()) {
            buffer.setLength(0); // Clear the buffer
            for(int i = 0 ; i < messages.length ; i ++) {
                if (messages[i] instanceof Double) {
                    buffer.append(String.format("%.2f", messages[i])); // round the double value to 2 decimal places
                } else {
                    buffer.append(String.valueOf(messages[i]));
                }
            }
