/*
 * ** Network and Service Differentiation Extensions to iQuantum 3.0 **
 *
...
public class InfoPacket implements Packet {
	...
	/** Whether the packet is going or returning, according to 
         * constants {@link  iQuantumTags#INFOPKT_SUBMIT}
         * and {@link  iQuantumTags#INFOPKT_RETURN}. */
	private int tag;
	...
	/** The level of service type. 
         ...
         the {@link CTask#netToS}? If yes, so the names would
         be standardized. */
	...
	private void init() {
		last = srcId;
		tag = iQuantumTags.INFOPKT_SUBMIT;
		bandwidth = -1;
		hopsNumber = 0;
		pingSize = size;
	...
	public String toString() {
		...
		for (int i = 0; i < entities.size(); i++) {
			...
			sb.append(iQuantum.getEntityName(resID) + "\t\t");
	 ...
	 * 
	 * @param time current simulation time, use {@link gridsim.iQuantum#clock()} to obtain this
	 * @pre time >= 0
	 * @post $none
         * 
	 */
	...
	public boolean setTag(int tag) {
		...
		switch (tag) {
			...
			case iQuantumTags.INFOPKT_RETURN:
				this.tag = tag;
				flag = true;
				break;
