...
public class Switch extends SimEntity {
	...
	public Switch(String name, int level, NetworkCDatacenter dc) {
		super(name);
		this.level = level;
		this.dc = dc;
	}
	...
	public void processEvent(SimEvent ev) {
		// Log.printLine(iQuantum.clock()+"[Broker]: event received:"+ev.getTag());
		switch (ev.getTag()) {
		// Resource characteristics request
			case iQuantumTags.Network_Event_UP:
				// process the packet from down switch or host
				processpacket_up(ev);
				break;
			case iQuantumTags.Network_Event_DOWN:
				// process the packet from uplink
				processpacket_down(ev);
				break;
			case iQuantumTags.Network_Event_send:
				processpacketforward(ev);
				break;

			case iQuantumTags.Network_Event_Host:
				processhostpacket(ev);
				break;
			// Resource characteristics answer
			case iQuantumTags.RESOURCE_Register:
				registerHost(ev);
				break;
	...
	protected void processpacket_down(SimEvent ev) {
		...
		iQuantum.cancelAll(getId(), new PredicateType(iQuantumTags.Network_Event_send));
		schedule(getId(), latency, iQuantumTags.Network_Event_send);
	...
	protected void processpacket_up(SimEvent ev) {
		...
		int recvVMid = hspkt.pkt.reciever;
		iQuantum.cancelAll(getId(), new PredicateType(iQuantumTags.Network_Event_send));
		schedule(getId(), switching_delay, iQuantumTags.Network_Event_send);
		if (level == NetworkConstants.EDGE_LEVEL) {
	...
	protected void processpacket(SimEvent ev) {
		...
		iQuantum.cancelAll(getId(), new PredicateType(iQuantumTags.Network_Event_UP));
		schedule(getId(), switching_delay, iQuantumTags.Network_Event_UP);
			...
			for (Entry<Integer, List<NetworkPacket>> es : downlinkswitchpktlist.entrySet()) {
				...
				if (!hspktlist.isEmpty()) {
					...
					while (it.hasNext()) {
						...
						this.send(tosend, delay, iQuantumTags.Network_Event_DOWN, hspkt);
					}
					hspktlist.clear();
				}
			}
			...
			for (Entry<Integer, List<NetworkPacket>> es : uplinkswitchpktlist.entrySet()) {
				...
				if (!hspktlist.isEmpty()) {
					...
					while (it.hasNext()) {
						...
						this.send(tosend, delay, iQuantumTags.Network_Event_UP, hspkt);
					}
					hspktlist.clear();
				}
			}
			...
			for (Entry<Integer, List<NetworkPacket>> es : packetTohost.entrySet()) {
				...
				if (!hspktlist.isEmpty()) {
					...
					while (it.hasNext()) {
						...
						// hs.packetrecieved.add(hspkt);
						this.send(getId(), hspkt.pkt.data / avband, iQuantumTags.Network_Event_Host, hspkt);
					}
					hspktlist.clear();
				}
			}
