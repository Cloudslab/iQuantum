...
/**
 * Implements the network layer in iQuantum. It reads a file in the <a href="http://www.cs.bu.edu/brite/user_manual/node29.html">BRITE format</a>,
 ...
 * in network traffic of iQuantum.
 * <p/>
 ...
 * Nevertheless, each iQuantum entity must be mapped to one (and only one) BRITE node to allow
...
public class NetworkTopology {
	...
	/**
         * The map between iQuantum entities and BRITE entities.
         * Each key is a iQuantum entity ID and each value the corresponding
         * BRITE entity ID.
         */
	 ...
	 * Adds a new link in the network topology.
         * The iQuantum entities that represent the source and destination of the link
         * will be mapped to BRITE entities.
	 * 
	 * @param srcId ID of the iQuantum entity that represents the link's source node
	 * @param destId ID of the iQuantum entity that represents the link's destination node
	...
	/**
	 * Maps a iQuantum entity to a BRITE node in the network topology.
	 * 
	 * @param iQuantumEntityID ID of the entity being mapped
	 * @param briteID ID of the BRITE node that corresponds to the iQuantum entity
	 * @pre iQuantumEntityID >= 0
	 * @pre briteID >= 0
	 * @post $none
	 */
	public static void mapNode(int iQuantumEntityID, int briteID) {
		if (networkEnabled) {
			try {
				// this iQuantum entity was already mapped?
				if (!map.containsKey(iQuantumEntityID)) {
					if (!map.containsValue(briteID)) { // this BRITE node was already mapped?
						map.put(iQuantumEntityID, briteID);
					} else {
						Log.printConcatLine("Error in network mapping. BRITE node ", briteID, " already in use.");
					}
				} else {
					Log.printConcatLine("Error in network mapping. iQuantum entity ", iQuantumEntityID,
							" already mapped.");
				}
			} catch (Exception e) {
				Log.printConcatLine("Error in network mapping. iQuantum node ", iQuantumEntityID,
	...
	/**
	 * Unmaps a previously mapped iQuantum entity to a BRITE node in the network topology.
	 * 
	 * @param iQuantumEntityID ID of the entity being unmapped
	 * @pre iQuantumEntityID >= 0
	 * @post $none
	 */
	public static void unmapNode(int iQuantumEntityID) {
		if (networkEnabled) {
			try {
				map.remove(iQuantumEntityID);
			} catch (Exception e) {
				Log.printConcatLine("Error in network unmapping. iQuantum node: ", iQuantumEntityID);
			}
		}
	}
